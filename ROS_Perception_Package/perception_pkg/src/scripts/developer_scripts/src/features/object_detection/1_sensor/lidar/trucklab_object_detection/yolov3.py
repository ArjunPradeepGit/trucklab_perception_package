...
def convolutional(input_layer, filters_shape, downsample=False, activate=True, bn=True, activate_type='leaky'):
    if downsample:
        input_layer = ZeroPadding2D(((1, 0), (1, 0)))(
            input_layer)  # syntax --> ((top_pad, bottom_pad), (left_pad, right_pad))
        padding = 'valid'
        strides = 2
    else:
        strides = 1
...
def Create_Yolo(input_size=416, channels=3, training=False, CLASSES=YOLO_TRUCKLAB_CLASSES):
...
def compute_loss(pred, conv, label, bboxes, i=0, CLASSES=YOLO_TRUCKLAB_CLASSES):
    NUM_CLASS = len(read_class_names(CLASSES))
