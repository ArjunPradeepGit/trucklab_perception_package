...
def main(args):
    ...
    PATH_FOLDER_BAGS = Path(ROOT_DIR / "developer_scripts" / "test_scripts" / "test_lidar_accuracy" / "bags")
    ...
    plt.title("LiDAR: Mean of range values vs Std of range values", fontsize=font_size+5)
    means_plot = plt.plot(range_means, range_stds, 'b+')
    plt.xlabel("Mean of range values (m)", fontsize=font_size)
    plt.ylabel("Standard deviations of range values (m)", fontsize=font_size)
    plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-tot_cap', '--tot_cap', help='Provide the number of total captures', type=int, default=7)
    parser.add_argument('-obj_name', '--obj_name', help='Provide the name of the object', type=str, default="lidar_error")
