...
'''
Script: Data reader for lidar captures
...
def main(args):
    ...
    PATH_FILE_DATA_CAPTURES = Path(ROOT_DIR / "data" / "data_captured_for_training_data" / "processed_data_files" / f"{args.obj_name}.csv")
    ...
    for cap_num in range(1, args.tot_cap+1):
        ...
        OPTI_MSG = bag_file.message_by_topic(topic_optitrack)
        df_optitrack = pd.read_csv(OPTI_MSG)

        x_mean = df_optitrack["pose.position.x"].mean()
        y_mean = df_optitrack["pose.position.y"].mean()
        r_x_mean = df_optitrack["pose.orientation.x"].mean()
        r_y_mean = df_optitrack["pose.orientation.y"].mean()
        r_z_mean = df_optitrack["pose.orientation.z"].mean()
        r_w_mean = df_optitrack["pose.orientation.w"].mean()
        r_mean = R.from_quat([r_x_mean, r_y_mean, r_z_mean, r_w_mean])
        theta_mean, _, _ = r_mean.as_euler('zxy', degrees=False)

        dict_capture = {range_cols[i]: [range_means[i]] for i in range(len(range_cols))}
        dict_capture.update({angle_cols[i]: [angle_means[i]] for i in range(len(angle_cols))})
        dict_capture["x"] = x_mean
        dict_capture["y"] = y_mean
        dict_capture["theta"] = theta_mean
        df_capture = pd.DataFrame([dict_capture])

        df_lidar_captures = pd.concat([df_lidar_captures, df_capture], ignore_index=True)

    df_lidar_captures.to_csv(PATH_FILE_DATA_CAPTURES)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-obj_name', '--obj_name', help='Provide the name of the object', type=str)
